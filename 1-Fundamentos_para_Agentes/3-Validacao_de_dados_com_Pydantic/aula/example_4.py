...
class User(BaseModel):
    model_config = {
        "extra": "forbid",      #rejeiita campos não declarods no modelos
    }
    __users__ = []              #variavel de classe, onde é armazenado todos os usuários criados
    name: str = Field(..., description="Name of the user")                  #Validação nome
    email: EmailStr = Field(..., description="Email address of the user")   #validação do e-mail
    friends: list[UUID4] = Field(                                           #cria uma lista para cada objeto, ou seja, para cada usuário
    ...
    signup_ts: Optional[datetime] = Field(
        default_factory=datetime.now, description="Signup timestamp", kw_only=True  #argumentos nomeados apenas, se posicional não funciona
    )
    id: UUID4 = Field(
        default_factory=uuid4, description="Unique identifier", kw_only=True
    )

    @field_serializer("id", when_used="json")                                   #Padrão de saída será o json
